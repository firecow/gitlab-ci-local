...
describe("Creating git data", () => {
    ...
    describe("only for git remote data", () => {
        describe("for working input", () => {
            test.each([
                `git@${remote.domain}:${remote.group}/${remote.project}.git`,
                `git@${remote.domain}/${remote.group}/${remote.project}.git`,
                `git@${remote.domain}:222/${remote.group}/${remote.project}.git`,
                `https://git:git@${remote.domain}/${remote.group}/${remote.project}.git`,
                `https://git:git@${remote.domain}:22/${remote.group}/${remote.project}.git`,
            ])("%s", async (url) => {
                when(mock)
                    .calledWith("git remote -v", expect.any(String))
                    .mockReturnValue({
