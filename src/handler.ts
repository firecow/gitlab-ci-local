...
export async function handler(argv: any, writeStreams: WriteStreams): Promise<ReadonlyMap<string, Job>> {
    ...
    if (argv.completion != null) {
        ...
        parser = await Parser.create({
            cwd, writeStreams, pipelineIid, showInitMessage: false, file: argv.file, home: argv.home, extraHosts: argv.extraHost,
        });
        ...
        const pipelineIid = await state.getPipelineIid(cwd);
        parser = await Parser.create({
            cwd, writeStreams, pipelineIid, file: argv.file, home: argv.home, extraHosts: argv.extraHost,
        });
