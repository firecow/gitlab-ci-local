    ...
    private async serviceHealthCheck (writeStreams: WriteStreams, service: Service, serviceIndex: number, serviceContanerLogFile: string) {
        ...
        try {
            ...
            await Promise.any(Object.keys(imageInspect[0].Config.ExposedPorts).map((port) => {
                ...
                const spawnCmd = [this.argv.containerExecutable, "run", "--rm", `--name=gcl-wait-for-it-${this.jobId}-${serviceIndex}-${portNum}`, "--network", `gitlab-ci-local-${this.jobId}`, "docker.io/willwill/wait-for-it", `${uniqueAlias}:${portNum}`, "-t", "30"];
